...
function Navbar() {
  ...
  const { searchTerm, setSearchTerm, wishlist } = useProduct();
  ...
  return (
    <nav className="bg-yellow-500 shadow-lg sticky top-0 z-50">
      <div className="container mx-auto flex items-center justify-between p-4 flex-wrap">
        {/* Logo */}
        <Link to="/" className="text-2xl font-extrabold text-white tracking-wide hover:scale-105 transition">MrShrek</Link>
        ...
        <div className="hidden md:flex items-center space-x-6 text-white font-medium">
          ...
          {user ? (
            ...
            <>
              <Link to="/login" className="flex items-center gap-1 hover:text-gray-200 transition"><LogIn size={18} /> Login</Link>
              <Link to="/signup" className="flex items-center gap-1 hover:text-gray-200 transition"><UserPlus size={18} /> Sign Up</Link>
            </>
          )}

          {/* Wishlist */}
          <Link to="/wishlist" className="relative flex items-center gap-1 hover:text-gray-200 transition">
            <Heart size={22} />
            <span>Wishlist</span>
            {wishlistCount > 0 && (
              <span className="absolute -top-2 -right-2 bg-red-600 text-xs px-2 py-0.5 rounded-full shadow-lg">{wishlistCount}</span>
            )}
          </Link>
          ...
          <Link to="/cart" className="relative flex items-center gap-1 hover:text-gray-200 transition">
            ...
            <span>Cart</span>
            {cartCount > 0 && (
              <span className="absolute -top-2 -right-2 bg-red-600 text-xs px-2 py-0.5 rounded-full shadow-lg">{cartCount}</span>
            )}
          </Link>
        </div>
      </div>
        ...
        <div className="md:hidden bg-yellow-500 px-4 pb-4">
          <div className="flex flex-col space-y-2">
            ...
            <Link to="/wishlist" className="relative flex items-center gap-1 hover:text-gray-200 transition">
              <Heart size={22} />
              <span>Wishlist</span>
              {wishlistCount > 0 && <span className="absolute -top-2 -right-2 bg-red-600 text-xs px-2 py-0.5 rounded-full shadow-lg">{wishlistCount}</span>}
            </Link>
            <Link to="/cart" className="relative flex items-center gap-1 hover:text-gray-200 transition">
              <ShoppingCart size={22} />
              <span>Cart</span>
              {cartCount > 0 && <span className="absolute -top-2 -right-2 bg-red-600 text-xs px-2 py-0.5 rounded-full shadow-lg">{cartCount}</span>}
            </Link>
          </div>
        </div>
      )}
